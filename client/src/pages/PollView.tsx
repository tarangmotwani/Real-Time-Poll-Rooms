...
export default function PollView() {
  ...
  // Initialize voter token + check vote state
  useEffect(() => {
    if (!pollId) return;
    ...
    const votedPolls = JSON.parse(
      localStorage.getItem(VOTED_POLLS_KEY) || "{}"
    );
  ...
  useEffect(() => {
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === VOTED_POLLS_KEY && pollId) {
        const votedPolls = JSON.parse(
          localStorage.getItem(VOTED_POLLS_KEY) || "{}"
        );
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-background p-4 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          This poll might have been deleted or the link is incorrect.
        </p>
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/30 py-12 px-4 flex items-center justify-center">
      <div className="max-w-3xl w-full space-y-8">
        ...
        <Card className="shadow-2xl border-primary/10 overflow-hidden bg-white/80 backdrop-blur-md">
            ...
            {!hasVoted ? (
              ...
              >
                  ...
                  {poll.options.map((option) => (
                    <div key={option.id}>
                      ...
                      <Label
                        ...
                        className="flex items-center justify-between w-full p-5 text-lg font-medium bg-secondary/30 border-2 border-transparent rounded-xl cursor-pointer hover:bg-secondary/60 hover:border-primary/20 peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5"
                      >
                        {option.text}
                        <CheckCircle2 className="w-6 h-6 opacity-0 peer-data-[state=checked]:opacity-100 text-primary" />
                      </Label>
                    </div>
                  ))}
                </RadioGroup>
                ...
                >
                  {isVoting ? (
                    <>
                      <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Casting
                      Vote...
                    </>
                  ) : (
                    "Submit Vote"
          ...
          {hasVoted && (
            <CardFooter className="bg-secondary/20 pt-6 flex justify-center border-t">
              <p className="text-muted-foreground text-sm font-medium">
                You voted for{" "}
                <span className="text-foreground font-bold">
                  {
                    poll.options.find((o) => o.id === votedOptionId)
                      ?.text
                  }
                </span>
              </p>
